...
systemctl() {
  ...
  case "$1" in
    restart)
      if [[ "$(read_setting PORT 0)" != 3200 ]]; then return 1; fi
...
curl() {
  ...
  [[ "$requested" == http://127.0.0.1:3200/api/health ]]
}
...
[[ "$(cat "$test_root/health.request")" == http://127.0.0.1:3200/api/health ]]
...
printf '# Managed by gate-crossex-arbitrage installer\n' > "$UNIT_FILE"
old_release=
old_unit_backup=
old_environment=
old_health_url=
