...
export function fixture(t, engineOptions = {}) {
  ...
  let store = createStore(directory); const options = { clock: () => now, catalogReader: async () => currentCatalog, feedReader: async () => { if (offline) throw new Error('offline'); return currentFeed; }, depthReader: async q => depth(q), fxReader: async () => currentFeed.fx, ...engineOptions };
