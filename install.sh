...
APP_DIR=/opt/gate-crossex-arbitrage
DATA_DIR_DEFAULT=/var/lib/gate-crossex-arbitrage
ENV_FILE=/etc/gate-crossex-arbitrage.env
SERVICE=gate-crossex-arbitrage
SERVICE_USER=gate-crossex-arbitrage
UNIT_FILE=/etc/systemd/system/gate-crossex-arbitrage.service
REPOSITORY=https://github.com/hxx344/gate-crossex-arbitrage.git
BRANCH=main
...
log() { printf '[gate-crossex-arbitrage] %s\n' "$*"; }
...
prepare_environment_rollback() {
  ...
  old_health_url=$(health_url "$(read_setting HOST 127.0.0.1 "$old_environment")" "$(read_setting PORT 3200 "$old_environment")")
}
restore_environment() {
...
rollback() {
  ...
  if [[ "$activation_started" == 1 ]]; then
    ...
    if [[ -n "$old_unit_backup" && -f "$old_unit_backup" ]]; then
      ...
      if [[ -f "$UNIT_FILE" ]] && grep -q '^# Managed by gate-crossex-arbitrage installer$' "$UNIT_FILE"; then rm -f -- "$UNIT_FILE"; fi
    fi
...
main() {
  ...
  ensure_tools
  mkdir -p /run/lock
  exec 9>/run/lock/gate-crossex-arbitrage-install.lock
  ...
  if [[ -f "$UNIT_FILE" ]]; then
    grep -q '^# Managed by gate-crossex-arbitrage installer$' "$UNIT_FILE" || fail '同名 systemd 服务已存在且不属于本安装脚本。'
  ...
  PORT_VALUE=$(read_setting PORT 3200)
  ...
  case "$DATA_PATH" in /var/lib/gate-crossex-arbitrage|/var/lib/gate-crossex-arbitrage/*|/srv/gate-crossex-arbitrage/*|/opt/gate-crossex-arbitrage-data|/opt/gate-crossex-arbitrage-data/*) ;; *) fail 'DATA_DIR 请使用 /var/lib/gate-crossex-arbitrage、/srv/gate-crossex-arbitrage/ 子目录或 /opt/gate-crossex-arbitrage-data，避免修改系统目录权限。' ;; esac
